...
namespace Assets.Nurface.VRInteraction.Scripts
{
    public class NvrInteractiveObject : MonoBehaviour {
        ...
        //Modified parts
        textTimer = GameObject.FindWithTag("GazeTimerText").GetComponent<Text>();
        textTimer.transform.localScale = Vector3.zero;
        textTimer.text = gazeClickTime.ToString("0");
    }
        ...
        if (interactionEnabled == false || clickWithGaze == false) {
            ...
            textTimer.transform.localScale = Vector3.one;
            textTimer.text = timer.ToString("0");
            MessagePopUp.PopUpMessage($"Look at the object for {gazeClickTime} seconds");
